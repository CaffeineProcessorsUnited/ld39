...
export class Trigger {
  ...
  trigger(action: string, data: any, context?: any): boolean {
    switch (action) {
      case "position":
        if (this.test(data.x, data.y)) {
          if (this.active === false) {
            return this.enter(context)
          } else {
            return this.each(context)
          }
        } else {
          if (this.active === true) {
            return this.exit(context)
          }
        }
        break
      case "keydown":
        if (this.test(data.x, data.y)) {
          if (this.onKeyDown.hasOwnProperty(data.keyCode)) {
            return (this.onKeyDown[data.keyCode] !== undefined) ? this.onKeyDown[data.keyCode].call(context) : false
          }
        }
        break
      case "keyup":
        if (this.test(data.x, data.y)) {
          if (this.onKeyUp.hasOwnProperty(data.keyCode)) {
            return (this.onKeyUp[data.keyCode] !== undefined) ? this.onKeyUp[data.keyCode].call(context) : false
          }
        }
        break
      case "keypress":
        if (this.test(data.x, data.y)) {
          if (this.onKeyPress.hasOwnProperty(data.keyCode)) {
            return (this.onKeyPress[data.keyCode] !== undefined) ? this.onKeyPress[data.keyCode].call(context) : false
          }
        }
        break
    }
